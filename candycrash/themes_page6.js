...
function showTopics(p, i) {
    ...
    topicNodes = svg.append("g")
      .attr("class", "nodes")
      .selectAll("g")
      .data(root_1.children, function(d) {return d.data.name; })
      .enter().append("g")
      .attr("class", "topics")
...
function showLeaves(p,i) {
  //////if any Leaves are displayed remove and re-add corresponding topic circle
  if (typeof hiddenTopic != undefined) {

    var oldLeaves = svg.select(".nodes").selectAll("g .stories").remove();
    oldLeaves.exit();

    var displayedTopics = svg.select(".nodes").selectAll("g").data(root_1.children, function(d) {return d.data.name; });

    var redisplayedTopic = displayedTopics.enter().append("g")
    .attr("class", "topics")
    .attr('transform', function(d) {return 'translate(' + [d.x , d.y] + ')'})
    .attr("pointer-events", "none")
    .on("mouseover", showLeaves);
    ...
    redisplayedTopic.append('circle')
      .style("fill", function(d) { return myCol(d.data.name); })
      .style("stroke", function(d) { return myCol(d.data.name); })
      .style("opacity", 0.6)
      .attr('r', function(d) { return d.value; });

    redisplayedTopic.append('text')
      .selectAll("tspan")
      .data(function(d) {return d.data.name.split(' ');})
      .enter().append("tspan")
      .text(d => d)
    ...
    redisplayedTopic.append("title")
      .text(function(d) {return d.data.name;});

    displayedTopics.exit();
    }

  d3.select(this).remove()
  var localX = this.__data__.x;
  var localY = this.__data__.y;
  hiddenTopic = this.children[2].innerHTML;
  var subroot = filterTree(root, hiddenTopic);
  ...
  newNodes.append("circle")
    .style("fill", function(d) {return myCol(hiddenTopic)})
    .style("opacity", 0.6)
    .attr('r', radLeaf);
...
function updateFunction(data, nodes) {
    ...
    if (simulation.alpha() === 1); {
      ...
      simulation = d3.forceSimulation()
        .force("x", d3.forceX().strength(0.001).x(width*0.5))
        .force("y", d3.forceY().strength(0.001).y(height*0.5))
        .force("charge", d3.forceManyBody().strength(5)) // Nodes attracted to one and other > 0
        .force("collide", d3.forceCollide().strength(0.1).radius(function(d){ return (d.value) }).iterations(4));// Force that avoids circle overlap
